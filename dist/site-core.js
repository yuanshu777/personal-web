...
function runBootSequence(boot){if(!boot||typeof boot.querySelectorAll!=='function')return;const count=typeof boot.querySelector==='function'?boot.querySelector('[data-rule-count]'):null,values=Array.from(boot.querySelectorAll('[data-calc]')||[]);if(!count&&!values.length)return;const schedule=(at,task)=>setTimeout(task,Math.max(0,at-performance.now()));let tick=0;const compute=()=>{tick+=1;const elapsed=Math.min(4600,performance.now()),rules=Math.min(2048,Math.floor(elapsed/4600*2048));if(count)count.textContent=String(rules).padStart(4,'0')+' / 2048';if(values.length===4){const wave=(Math.sin(tick*.37)+Math.cos(tick*.19))*.5;values[0].textContent=(wave*1.61803399).toFixed(8);values[1].textContent=[17,31,47].map((seed,index)=>String(Math.abs(Math.floor(Math.sin(tick*.13+index)*9999+seed))).padStart(4,'0')).join('·');values[2].textContent=String(tick*137).padStart(6,'0');values[3].textContent=(100*(1-Math.exp(-tick/29))).toFixed(6)+'%';}};compute();const calculator=setInterval(compute,72);schedule(4200,()=>boot.dataset.phase='converge');schedule(5000,()=>boot.dataset.eye='open');schedule(5850,()=>boot.dataset.identity='locked');schedule(7100,()=>clearInterval(calculator));}
function finishBoot(){const boot=document.querySelector('#boot-screen'),stylesheet=document.querySelector('#site-css'),minimum=7200;let ready=false;runBootSequence(boot);const reveal=()=>{if(ready)return;ready=true;setTimeout(()=>requestAnimationFrame(()=>requestAnimationFrame(()=>{document.documentElement.classList.add('app-ready');setTimeout(()=>boot?.remove(),760)})),Math.max(0,minimum-performance.now()))};if(!stylesheet||stylesheet.media==='all')reveal();else{stylesheet.addEventListener('load',reveal,{once:true});stylesheet.addEventListener('error',reveal,{once:true});}}
