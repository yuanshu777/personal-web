...
function runBootSequence(boot){if(!boot||typeof boot.querySelectorAll!=='function')return;const steps=[...boot.querySelectorAll('.boot-step')],moments=[200,1450,2700,3950],schedule=(at,task)=>setTimeout(task,Math.max(0,at-performance.now()));moments.forEach((at,index)=>schedule(at,()=>steps.forEach((step,stepIndex)=>{step.classList.toggle('is-active',stepIndex===index);step.classList.toggle('is-complete',stepIndex<index)})));schedule(4700,()=>boot.dataset.eye='open');}
function finishBoot(){const boot=document.querySelector('#boot-screen'),stylesheet=document.querySelector('#site-css'),minimum=6400;let ready=false;runBootSequence(boot);const reveal=()=>{if(ready)return;ready=true;setTimeout(()=>requestAnimationFrame(()=>requestAnimationFrame(()=>{document.documentElement.classList.add('app-ready');setTimeout(()=>boot?.remove(),620)})),Math.max(0,minimum-performance.now()))};if(!stylesheet||stylesheet.media==='all')reveal();else{stylesheet.addEventListener('load',reveal,{once:true});stylesheet.addEventListener('error',reveal,{once:true});}}
