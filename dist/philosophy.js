...
function updateSceneLabels(){const act=acts[currentAct];document.querySelector('#scene-counter').textContent=String(currentAct+1).padStart(2,'0')+' / 09';document.querySelector('#scene-kind').textContent=t(act.kind);document.querySelector('#scene-symbol').textContent=language==='zh'?['世界 → 身体 → 体验','可能 / 不可能','世界 ⊂ ？','W₁(t) · W₂(t) · W₃(t)','世界 → 局部过程 → 我','能力 / 选择 / 认识','信息 → 体验？','W → I → model(W) ⊂ W','经验 / 推论 / 假设'][currentAct]:act.symbol;document.querySelector('#scene-caption').textContent=t(act.caption);document.querySelector('#scene-accessibility').textContent=t(act.caption);}
...
function update(){
  ...
  if(!readingMode){
    ...
    canvas.style.opacity=photo?'0':'1';
...
render();
function sceneFailure(){canvas.hidden=true;document.querySelector('#scene-fallback').hidden=false;}
